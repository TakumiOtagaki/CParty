...
PROMPT_FILE="$ROOT_DIR/documents/ralph-loop/rprompt.md"
PRD_FILE="$ROOT_DIR/documents/ralph-loop/prd.json"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  OUTPUT=$(echo "$PROMPT_CONTENT" | bash -lc "$AGENT_CMD" 2>&1 | tee /dev/stderr) || true
