...
void W_final_pf::compute_WMBP(cand_pos_t i, cand_pos_t j, sparse_tree &tree) {
    cand_pos_t ij = index[i] + j - i;
    pf_t contributions = 0;
    const scfg::PartFuncModeConfig mode_config{expPB_penalty, TURN};
    ...
    if (rules.pair_at(j) < 0 && rules.pair_at(i) >= 0) {
        rules.for_each_split(i, j, [&](cand_pos_t l) {
            if (rules.has_valid_inner_arc_split(i, l, j, n) && rules.parent_within_interval_and_turn(i, l, j)) {
                const cand_pos_t bp_il = rules.border_bp(i, l);
                pf_t m4 = get_BE(i, rules.pair_at(i), bp_il, rules.pair_at(bp_il), tree) * get_energy_WI(bp_il + 1, l - 1) * get_energy_VP(l, j);
