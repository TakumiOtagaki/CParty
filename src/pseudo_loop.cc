...
void pseudo_loop::compute_VP(cand_pos_t i, cand_pos_t j, sparse_tree &tree) {
    ...
    if (rules.parent_index(i) < rules.parent_index(j) && rules.parent_index(j) > 0 && b_ij >= 0 && bp_ij >= 0 && Bp_ij < 0) {
    ...
    // Hosna April 9th, 2007
    // checking the borders as they may be negative
    if (rules.parent_index(i) > 0 && rules.parent_index(j) > 0 && Bp_ij >= 0 && B_ij >= 0 && b_ij >= 0 && bp_ij >= 0) {
...
void pseudo_loop::back_track(std::string structure, minimum_fold *f, seq_interval *cur_interval, sparse_tree &tree) {
    ...
    case P_WMB: {
        back_track_wmb(cur_interval, tree);
    } break;

    case P_WMBW: {
        back_track_wmbw(cur_interval, tree);
    } break;
    case P_WMBP: {
        back_track_wmbp(cur_interval, tree);
    } break;
    case P_VP: {
    ...
    case P_VPL: {
        back_track_vpl(cur_interval, tree);
    } break;

    case P_VPR: {
        back_track_vpr(cur_interval, tree);
    } break;
    case P_WI: {
        back_track_wi(cur_interval, tree);
    } break;
    case P_BE: {
        back_track_be(cur_interval, tree);
    } break;
    case P_WIP: {
        back_track_wip(cur_interval, tree);
    } break;
    }
}
