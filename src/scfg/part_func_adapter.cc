...
class LocalWContext final : public PartFuncWContext {
  public:
    ...
    pf_t get_WMB(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMB(i, j); }
...
class LocalVMContext final : public PartFuncVMContext {
  public:
    ...
    pf_t get_WM(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WM(i, j); }
    pf_t get_WMv(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMv(i, j); }
    pf_t get_WMp(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMp(i, j); }
...
class LocalWIPContext final : public PartFuncWIPContext {
  public:
    ...
    pf_t get_V(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy(i, j); }
    pf_t get_WMB(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMB(i, j); }
    pf_t get_WIP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WIP(i, j); }
...
class LocalVPLContext final : public PartFuncVPLContext {
  public:
    ...
    pf_t get_VP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_VP(i, j); }
    pf_t expcp_pen(cand_pos_t length) const override { return PartFuncAdapterAccess::expcp_pen(owner_)[length]; }
...
class LocalVPContext final : public PartFuncVPContext {
  public:
    ...
    pf_t get_VP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_VP(i, j); }
    pf_t get_WIP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WIP(i, j); }
    pf_t get_VPL(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_VPL(i, j); }
    pf_t get_VPR(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_VPR(i, j); }
    pf_t get_V(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy(i, j); }
...
class LocalWMBWContext final : public PartFuncWMBWContext {
  public:
    ...
    pf_t get_WMBP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMBP(i, j); }
    pf_t get_WI(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WI(i, j); }
...
class LocalWMBPContext final : public PartFuncWMBPContext {
  public:
    ...
    pf_t get_WMBP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMBP(i, j); }
    pf_t get_WMBW(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WMBW(i, j); }
    pf_t get_VP(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_VP(i, j); }
    pf_t get_WI(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy_WI(i, j); }
    pf_t get_V(cand_pos_t i, cand_pos_t j) override { return owner_.get_energy(i, j); }
...
void compute_W_restricted(W_final_pf &owner, sparse_tree &tree) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            compute_W_restricted_core(ctx, all, tree, config);
        } else {
...
pf_t compute_VM_restricted(W_final_pf &owner, cand_pos_t i, cand_pos_t j, std::vector<int> &up) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            return compute_VM_restricted_core(ctx, all, i, j, up, config);
        }
...
void compute_V_restricted(W_final_pf &owner, cand_pos_t i, cand_pos_t j, sparse_tree &tree) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            compute_V_restricted_core(ctx, all, i, j, tree, config);
        } else {
...
void compute_WI_restricted(W_final_pf &owner, cand_pos_t i, cand_pos_t j, sparse_tree &tree) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            compute_WI_restricted_core(ctx, all, i, j, tree, config);
        } else {
...
void compute_WMv_WMp_restricted(W_final_pf &owner, cand_pos_t i, cand_pos_t j, std::vector<Node> &tree) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            compute_WMv_WMp_restricted_core(ctx, all, i, j, tree, config);
        } else {
...
void compute_WM_restricted(W_final_pf &owner, cand_pos_t i, cand_pos_t j, sparse_tree &tree) {
    ...
    if (config.use_rules) {
        if (config.use_inside_core) {
            compute_WM_restricted_core(ctx, all, i, j, tree, config);
        } else {
